...
export default async function PlatformPage({ params }: { params: Promise<{ locale: Locale }> }) {
    ...
    return (
        <main className="relative min-h-screen pt-32 pb-20 overflow-hidden bg-black text-white">
            ...
            {/* Hero */}
            <section className="relative z-10 container mx-auto px-6 mb-24 max-w-6xl text-center">
                ...
                <p className="text-xl text-gray-300 mb-12">{content.subtitle}</p>
                <TextBlock
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {content.capabilities.map((cap, i) => (
                        <div key={i} className="p-8 border border-yellow-500/20 bg-yellow-900/5 backdrop-blur-sm rounded-lg">
                            ...
                            <p className="text-gray-300 mb-6">{cap.description}</p>
            ...
            <section className="relative z-10 container mx-auto px-6 mb-24">
                <h2 className="text-3xl font-bold text-white mb-12 text-center">{ui.useCases}</h2>
                    ...
                    {content.useCases.map((useCase, i) => (
                        <div key={i} className="p-8 border border-yellow-500/20 bg-yellow-900/5 backdrop-blur-sm rounded-lg">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <h4 className="text-xs uppercase tracking-widest text-gray-500 mb-1">{ui.theChallenge}</h4>
                                    <p className="text-gray-300 text-sm">{useCase.challenge}</p>
                                </div>
                                <div>
                                    <h4 className="text-xs uppercase tracking-widest text-gray-500 mb-1">{ui.ourSolution}</h4>
